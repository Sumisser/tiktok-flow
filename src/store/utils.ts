...
- **动态张力 (视频生成提示词)**：
    ...
    - 运镜配合：镜头运动（推拉、跟随、环绕）应服务于突出主体动态，而非替代主体动态。
    - 节奏把控：动作可以缓慢优雅，但不能"静止"；追求"时间流逝中的生命力"，而非"静态画面的微抖"。
    - 示例：
        - ❌ 差例："镜头缓慢推进，光影变化"（无主体动态）
        - ✅ 好例："镜头缓慢推进，人物抬起手触碰雨滴，雨水顺着指尖滑落，衣袂在微风中轻轻飘动"
  ...
  "storyboard": [
    {
      "shot_number": 0,
      "script": "[封面]",
      "image_prompt": "(为视频封面设计一张极具吸引力的静态画面，融合视频核心主题与视觉美学)",
      "video_prompt": "-"
    },
    {
      "shot_number": 1,
      "script": "(第一段文案)",
      "image_prompt": "电影感广角镜头，描述场景氛围... [风格融合]",
      "video_prompt": "人物缓步走入画面，衣袂随风轻摆，镜头跟随横摇，雨滴落在地面泛起涟漪..."
    },
    ...
  ]
}
...
5. **JSON 格式**：必须返回合法的标准 JSON 格式，不要包含 Markdown 代码块标记（如 \`\`\`json），直接返回 JSON 字符串即可。
    `,
    input: '',
    output: '',
    status: 'pending',
